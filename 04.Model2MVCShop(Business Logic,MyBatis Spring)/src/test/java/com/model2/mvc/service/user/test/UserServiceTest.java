...
//		user.setEmail("test@test.com");
		...
		user = userService.getUser("user13");

		//==> console 확인
		System.out.println(user);
		
		//==> API 확인
		Assert.assertEquals("user13", user.getUserId());
		Assert.assertEquals("SCOTT", user.getUserName());
		Assert.assertEquals("1313", user.getPassword());
		Assert.assertEquals(null, user.getPhone());
		Assert.assertEquals(null, user.getAddr());
