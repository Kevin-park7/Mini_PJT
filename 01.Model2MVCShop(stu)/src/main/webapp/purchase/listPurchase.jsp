	...
	<tr>
		<td align="center">
		 <%
					if(((curPage-1)/5)!=0){
					%>
					<a href="/listPurchase.do?page=<%=((curPage-1)/5)*5 %>&searchCondition=<%= searchVO.getSearchCondition() %>&searchKeyword=<%=searchVO.getSearchKeyword() %>&menu=manage">◀ 이전 </a>
					<% 
					}
					%>
					...
					<%
					 ...
					 for(int i=((curPage-1)/5)*5+1;i<=(((curPage-1)/5)+1)*5;i++){
						...
						if(i <= totalPage){
					%>
						<a href="/listPurchase.do?page=<%=i%>&searchCondition=<%= searchVO.getSearchCondition() %>&searchKeyword=<%=searchVO.getSearchKeyword() %>&menu=manage"><%=i %> </a>
					<%
						}
						}
					 if(((curPage-1)/5) < ((totalPage-1)/5)){
					%>	
					<a href="/listPurchase.do?page=<%=(((curPage-1)/5)+1)*5+1 %>&searchCondition=<%= searchVO.getSearchCondition() %>&searchKeyword=<%=searchVO.getSearchKeyword() %>&menu=manage">이후 ▶ </a>
					<%} %>
		</td>
	</tr>
</table>
